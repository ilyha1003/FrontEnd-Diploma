...
const fillListElement = (apiData) => {
    apiData.forEach((el) => {
        ...
        listElement.classList.add('trips-list__item');

        const tripsPreview = document.createElement('div');
        tripsPreview.classList.add('recently-trips-preview');

        const previewImage = document.createElement('img');
        previewImage.src = "/preview1.png";

        const price = document.createElement('div');
        price.classList.add('recently-trips-preview-price');
        price.innerText = '1900$'

        tripsPreview.append(previewImage, price);

        const tripsContent = document.createElement('div');
        tripsContent.classList.add('recently-trips-list-item-wrapper');

        const tripHeader = document.createElement('span');
        tripHeader.classList.add('recently-trips-list-item-header');
        tripHeader.innerText = el.title;

        const ratingField = document.createElement('div');
        ratingField.classList.add('recently-trips-rating-field');

        const ratingList = document.createElement('ul');
        ratingField.classList.add('recently-trips-rating-field-stars');

        for(let i = 0; i < 5; i++) {
            const ratingListElement = document.createElement('li');
        ...
        ratingNumber.classList.add('recently-trips-rating-field-number');
        ratingNumber.innerText = '5.0';

        ratingField.append(ratingList, ratingNumber);

        const descriptionField = document.createElement('div');
        descriptionField.classList.add('recently-trips-description-field');

        const daysDescription = document.createElement('div');
        daysDescription.classList.add('recently-trips-description');
        ...
        const daysDescriptionContent = document.createElement('span');
        daysDescriptionContent.classList.add('recently-trips-description-content');
        daysDescriptionContent.innerText = el.days;

        daysDescription.append(daysDescriptionContent);

        const placesDescription = document.createElement('div');
        placesDescription.classList.add('recently-trips-description');
        ...
        const placesDescriptionContent = document.createElement('span');
        placesDescriptionContent.classList.add('recently-trips-description-content');
        placesDescriptionContent.innerText = el.places;

        placesDescription.append(placesDescriptionContent);

        const countriesDescription = document.createElement('div');
        countriesDescription.classList.add('recently-trips-description');
        ...
        const countriesDescriptionContent = document.createElement('span');
        countriesDescriptionContent.classList.add('recently-trips-description-content');
        countriesDescriptionContent.innerText = el.days;
        ...
        const tripDescription = document.createElement('span');
        tripDescription.classList.add('recently-trips-content');
        tripDescription.innerText = 'From colourful Copenhagen to sophisticated Stockholm, this trip gives you 12 days of pure Scandinavian magic.';

        tripsContent.append(tripHeader, ratingField, descriptionField, tripDescription);

        const tripDetails = document.createElement('a');
        tripDetails.classList.add('hero-go-button');
        tripDetails.classList.add('recently-trips-small-button');
        tripDetails.innerText = 'View details';

        listElement.append(tripsPreview, tripsContent, tripDetails);

        tripsList.append(listElement);
